# xArm Simulation Dataset - RLDS Converter
...
The `xArmSimDataset` class processes robot simulation episodes containing:
- RGB camera images
...
Change the dataset path in `xarm_sim_dataset.py`:
...
Build the dataset:
```bash
cd /path/to/xarm_sim_dataset/
tfds build xarm_sim_dataset --data_dir=~/tfds_data --overwrite
```
